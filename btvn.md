...
- Yêu cầu 1 (3.0) **1 đối tượng chính**: Ticket (GET ALL/ONE/POST - 1 điểm/action), gửi request với Postman hoặc REST client khác bao gồm tất các trường hợp trả về khác nhau như đặc tả bên trên
...
- Yêu cầu 5 (1.0) **Phân trang danh sách**: Hiển thị thông tin trang hiện tại, nút chọn trang bất kỳ, nút trang trước, trang sau. Mặc định 5 ticket/trang. Có thể thêm tham số `limit` tùy chọn (không tính điểm)
